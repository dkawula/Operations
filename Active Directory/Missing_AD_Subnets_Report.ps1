...
[CmdletBinding()]
PARAM (
    [Parameter(Mandatory = $false, HelpMessage = "You must specify the Sender Email Address")]
    [ValidatePattern("[a-z0-9!#\$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#\$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?")]
    [String]$EmailFrom,

    [Parameter(Mandatory = $false, HelpMessage = "You must specify the Destination Email Address")]
    [ValidatePattern("[a-z0-9!#\$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#\$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?")]
    [String[]]$EmailTo,

    [Parameter(Mandatory = $false, HelpMessage = "You must specify the Email Server to use (IPAddress or FQDN)")]
    [String]$EmailServer,
...
PROCESS {
    ...
    FINALLY {
        ...
        $FinalEmailBody | Out-File ".\output\$ForestName-$dateformat-MissingADReport.html" -Force
        $emailbody = get-content ".\output\$ForestName-$dateformat-MissingADReport.html" -Raw
        ...
        $EmailFrom = "AD_Missing_Subnet_Report_NoReply@blah.com"

        $EmailTo = "Helpdesk@blah.com"
...
#Fore Mail to go with Hard Coded Parameters for now
    Send-MailMessage -From $EmailFrom -To $EmailTo -SmtpServer $SMTPServer -Credential $credential -Port 587 -Subject "AD Missing SubnetsReport: $StorageNode" -Encoding UTF8 -BodyAsHtml -Body $EmailBody

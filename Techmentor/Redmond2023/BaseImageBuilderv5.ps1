...
Author - Thomas Rayner
    ...
    08-30-22 DK - This Script has been updated for Windows Server 2022 and has Switches to choose the editions of Windows for the Lab
    Gold VHDx Files Reuqired
    ...
    Just Download the ISO's or Windows Server 2016,2019 and 2022 from your MSDN site
...
( 
    ...
    [Parameter(Mandatory)]
    [string]
    $adminPassword = 'C@ngeme2019!!@@##',

    [Parameter(Mandatory=$false)] 
    [SWITCH]$IncludeWindows2022GUI, 

    [Parameter(Mandatory=$false)] 
    [SWITCH]$IncludeWindows2022Core,
    ...
    [Parameter(Mandatory=$false)]
    [string]
    $WindowsKey = '3B9T9-XVNGH-DH8RP-WT88K-KQDFD'
...

{
    ...
    #Build the Windows 2022 Core Base VHDx for the Lab
            ...
            if (!(Test-Path "$($BaseVHDPath)\VMServerBase2022.vhdx")) 
                        {
...
Write-Log 'Host' 'Locate the Windows Windows Server 2019 ISO'
...
Write-Log 'Host' 'Locate the Windows Windows Server 2022 ISO'

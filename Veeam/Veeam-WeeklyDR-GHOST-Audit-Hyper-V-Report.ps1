...
<#>
$Replica = Get-VBRReplica -Name "BLAH"
$Replica
$RestorePoints = $Replica | Get-VBRRestorePoint
$RestorePoints | Select VMName | Sort-Object 
$RestorePoints | gm
$RestorePoints.iscorrupted
</#>

$allJobs = @()
foreach ($job in Get-VBRReplica |  Where-Object {$_.Name -notlike "*Replica*" }) {
   
   #$Objects = $Job | Get-VBRJobObject 
   $Objects = $Job | Get-VBRRestorePoint
  
   foreach ( $obj in $objects ) {
    $addJob = New-Object -TypeName PSObject
    $addJob | Add-Member -MemberType NoteProperty -Name "JobName" -value $job.name
    $addJob | Add-Member -MemberType NoteProperty -Name "VM" -value $obj.VMName
    $addJob | Add-Member -MemberType NoteProperty -Name 'HyperVReplica' -Value $job.IsHVReplica
    #$addjob | Add-Member -MemberType NoteProperty -Name 'IsConsistent' -Value $job.IsConsistent
    #$addjob | Add-Member -MemberType NoteProperty -Name 'IsCorrupted' -Value $job.IsCorrupted
    #$addjob | Add-Member -MemberType NoteProperty -Name 'RepoPath' -Value $job.GetRepository().FullPath.root
    #$addjob | Add-Member -MemberType NoteProperty -Name 'CreationTime' -Value $job.CreationTime
...
$VMInfo = foreach ($vm in $missing) {
     ...
     $reportObject = "" | Select-Object ComputerName,text
     $ReportObject.ComputerName =  ($vm.Value[1]).ToString()
     $ReportObject.text = " Has NOT been Replicated to GHOST"
...
#$alljobs.Result
  ...
  $VMInfo = foreach ($vm in $alljobs) {
     ...
     $reportObject1 = "" | Select-Object ComputerName,text,Job,Result1,LastRun
     $ReportObject1.ComputerName =  $vm.vm
     $ReportObject1.text = "is protected by GHOST Replica Job     "
...
$body = ConvertTo-Html -Body "<H1>Company Labs Veeam GHOST Audit Report </H1><H3> Weekly DR GHOST Audit </H3> <H3>Total Unprotected VM's $missingcount</H3><H3> Total Protected VM's $ProtectedCount</H3><H1>Unprotected VM's</H1>$($UnProtectedVMs | Convertto-Html -Property * -Fragment) <H1> Protected VM's on GHOST  </H1> $($ProtectedVMs | Convertto-Html -Property * -Fragment) <H3> Report Generated on $Date  </H3> " -Title "Veeam Replica Weekly Audit" -Head $Header |out-file .\Veeam-Weekly-DRReplica-Report.Html
...
$EmailFrom = "Operations-No-Reply@BLAH.com"
$EmailTo = "BLAH@BLAH.com"
$Subject = "Veeam GHOST Audit Weekly Report"
